...
def main():
    argument_spec = dict(
        ...
        public_access_block_configuration=dict(type='str', required=False),
    ...
    if state == 'create':
        ...
        # How do we tell if it's a no-op?  Is it always changed?
        ...
        if response['ProgressEvent']['OperationStatus'] is not 'unknown':
            changed = True

    def format_list(response):
        result = list()
        for each in response['ResourceDescriptions']:
            identifier = each['Identifier']
            # Convert the Resource Properties from a str back to json
            properties = json.loads(each['Properties'])
            bucket = dict()
            bucket[identifier] = properties
