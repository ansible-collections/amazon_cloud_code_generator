...
def main():
    ...
    argument_spec['destination_config'] = {'type': 'dict', 'options': {'on_failure': {'type': 'dict', 'options': {'destination': {'type': 'str'}}}}}
    argument_spec['enabled'] = {'type': 'bool'}
    argument_spec['event_source_arn'] = {'type': 'str'}
    argument_spec['filter_criteria'] = {'type': 'dict', 'options': {'filters': {'type': 'list', 'elements': 'dict', 'options': {'pattern': {'type': 'str'}}}}}
    ...
    argument_spec['source_access_configurations'] = {'type': 'list', 'elements': 'dict', 'options': {'type': {'type': 'str', 'choices': ['BASIC_AUTH', 'CLIENT_CERTIFICATE_TLS_AUTH', 'SASL_SCRAM_256_AUTH', 'SASL_SCRAM_512_AUTH', 'SERVER_ROOT_CA_CERTIFICATE', 'VIRTUAL_HOST', 'VPC_SECURITY_GROUP', 'VPC_SUBNET']}, 'uri': {'type': 'str'}}}
    argument_spec['tumbling_window_in_seconds'] = {'type': 'int'}
    argument_spec['function_response_types'] = {'type': 'list', 'elements': 'str', 'choices': ['ReportBatchItemFailures']}
    argument_spec['self_managed_event_source'] = {'type': 'dict', 'options': {'endpoints': {'type': 'dict', 'options': {'kafka_bootstrap_servers': {'type': 'list', 'elements': 'str'}}}}}
    argument_spec['amazon_managed_kafka_event_source_config'] = {'type': 'dict', 'options': {'consumer_group_id': {'type': 'str'}}}
    argument_spec['self_managed_kafka_event_source_config'] = {'type': 'dict', 'options': {'consumer_group_id': {'type': 'str'}}}
    ...
    required_if = [
        ['state', 'present', ['id', 'function_name'], True],['state', 'absent', ['id'], True],['state', 'get', ['id'], True]
    ]
    mutually_exclusive = [
    ...
    params['amazon_managed_kafka_event_source_config'] = module.params.get('amazon_managed_kafka_event_source_config')
    params['batch_size'] = module.params.get('batch_size')
    params['bisect_batch_on_function_error'] = module.params.get('bisect_batch_on_function_error')
    ...
    params['maximum_batching_window_in_seconds'] = module.params.get('maximum_batching_window_in_seconds')
    params['maximum_record_age_in_seconds'] = module.params.get('maximum_record_age_in_seconds')
    ...
    params['self_managed_kafka_event_source_config'] = module.params.get('self_managed_kafka_event_source_config')
    params['source_access_configurations'] = module.params.get('source_access_configurations')
    params['starting_position'] = module.params.get('starting_position')
    params['starting_position_timestamp'] = module.params.get('starting_position_timestamp')
    params['topics'] = module.params.get('topics')
    params['tumbling_window_in_seconds'] = module.params.get('tumbling_window_in_seconds')
    ...
    create_only_params = ['event_source_arn', 'starting_position', 'starting_position_timestamp', 'self_managed_event_source', 'amazon_managed_kafka_event_source_config', 'self_managed_kafka_event_source_config']
